...
def create_survival_figure(results_df, proportions_df, wilcoxon_results_df, modality_fractions_df, output_path=None):
    ...
    hue_order = ['all', 'no AT', 'AST & ART','ART only', 'AST only']
    ...
    axes['a'].set_xlabel('Year of diagnosis', fontsize=xylabelsize)
    axes['a'].set_ylabel('Fraction of\npatients', fontsize=xylabelsize)
    axes['a'].set_yticks([0.0, 0.25, 0.5])
    axes['a'].set_ylim(bottom=0.0)
    axes['a'].set_title('Fractions of patients\nby AT modality and year', fontsize=titlefontsize)
    ...
    for patch in axes['b'].patches:
        patch.set_edgecolor(patch.get_facecolor())
        patch.set_linewidth(2.0)
    axes['b'].set_xlabel('Year of diagnosis', fontsize=xylabelsize)
    axes['b'].set_ylabel('5-year survival\nfraction', fontsize=xylabelsize)
    axes['b'].set_yticks([0.0, 0.2, 0.4, 0.6, 0.8])
    axes['b'].set_title('5-year survival fractions\nby AT modality and year', fontsize=titlefontsize)
    axes['b'].legend(title='AT modality', loc='lower center', bbox_to_anchor=(0.5, 1.55), ncol=2, fontsize=legendfontsize, title_fontsize=legendtitlefontsize)
    ...
    sns.barplot(data=proportions_df, x='embedding_size', y='proportion_followed', hue='modality', 
                ...
                palette=sns.color_palette("husl", 5)[:-1], gap=0.2)
    axes['c'].set_xlabel('Embedding size', fontsize=xylabelsize)
    axes['c'].set_ylabel('Fraction POME-consistent', fontsize=xylabelsize)
    axes['c'].set_title('Fractions of patients with POME-\nconsistent AT modality (2019)', fontsize=titlefontsize)
    axes['c'].legend(title='AT modality', loc='lower left', fontsize=legendfontsize, title_fontsize=legendtitlefontsize, ncol=2)
    ...
    for mod in ['No therapy', 'Systemic and radiotherapy', 'Only radiotherapy', 'All']:
        ...
        sns.boxplot(data=mod_subset, x='embedding_size', y='survived_frac', hue='suggestion_followed', fill=False, ax=axes[label], 
                    gap=0.2)
        axes[label].set_xlabel('Embedding size', fontsize=xylabelsize)
        axes[label].set_ylabel('5-year survival fraction', fontsize=xylabelsize)
        axes[label].set_yticks([0.5, 0.6, 0.7, 0.8, 0.9,1.0])
        mod_label = mod2description_map[mod]
        if mod_label == 'all':
            mod_label = 'all AT modalities'
        axes[label].set_title(f'5-year survival fractions\n(2019, {mod_label})', fontsize=titlefontsize)
        axes[label].legend(title='AT modality\nPOME-consistent', fontsize=legendfontsize, title_fontsize=legendtitlefontsize, loc='lower left')
        axes[label].tick_params(labelsize=ticklabelsize)
            ...
            for idx, emb_size in enumerate(embedding_sizes_sorted):
                ...
                if not pval_row.empty:
                    ...
                    if pd.notna(pval):
                        ...
                        axes[label].text(idx, y_pos, pval_text, ha='center', fontsize=pvalfontsize)
        
        # Add panel labels
        axes[label].text(0.0, 1.0, label, transform=axes[label].transAxes + offset, fontsize=labelfontsize, fontweight='bold')
