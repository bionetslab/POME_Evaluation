...
Metrics: MAE and MAPE (``mre_cont``) on the continuous entries and accuracy on
the categorical entries. One CSV is written per (dataset, discretization, bins):
...
def save_result_to_dict(results_dict, num_run, na_ratio, tool_name, mae_cont,
                        acc_cat, mre_cont, dim=-1, bins=-1, discretization=""):
